...
public class WebHookCallbackHandler {
    ...
    public void run(InputStream input, OutputStream output) {
        ...
        if (callback.getString("ref").equals("refs/heads/test_version")) {
            IoUtil.writeUtf8(output, true, "This is a test callback, so ignore it");
            return;
        }

        //只要push事件
        if (!callback.getString("hook_name").equals("push_hooks")) {
            IoUtil.writeUtf8(output, true, "This is not a push event, so ignore it");
            return;
        }

        //打印提交信息
        System.out.println("hook_id = " + callback.getString("hook_id"));
        System.out.println("提交记录：");
        JSONArray commits = callback.getJSONArray("commits");
        for (int i = 0; i < commits.size(); i++) {
            JSONObject commit = commits.getJSONObject(i);
            String id = commit.getString("id");
            String committerName = commit.getJSONObject("committer").getString("name");
            String message = commit.getString("message");
            String timestamp = commit.getString("timestamp");
            System.out.println(timestamp + " " + id + " " + committerName + " " + message);
        }


        String repoName = callback.getJSONObject("repository").getString("name");
        //同步指定仓库
        System.out.println("同步指定仓库：" + repoName);
